...
summary: Fortgeschrittene Dashboard-Funktionen einschließlich Widget-Anpassung, Abfragefilterung, Variablenkonfiguration, Datentransformationen und Formatierungsoptionen für die Erstellung leistungsstarker, flexibler Visualisierungen gehören zum erweiterten Funktionsumfang. 
...
[role="lead"]
Das volle Potenzial Ihrer Dashboards lässt sich mit erweiterten Funktionen und Anpassungsoptionen ausschöpfen. Die Konfiguration von Widget-Abfragen, die Anwendung von Datentransformationen, die Nutzung von Variablen für dynamische Filterung sowie die Implementierung anspruchsvoller Formatierungen ermöglichen hochwirksame, maßgeschneiderte Visualisierungen für den Bedarf an Infrastrukturüberwachung.
...
== Gruppieren, Identifizieren und Aggregieren
...
Die in einem Widget angezeigten Daten werden aus den während der Erfassung gesammelten zugrunde liegenden Datenpunkten gruppiert (manchmal auch als zusammengefasst bezeichnet). Wenn Sie beispielsweise über ein Liniendiagramm-Widget verfügen, das die Speicher-IOPS im Zeitverlauf anzeigt, möchten Sie möglicherweise für jedes Ihrer Rechenzentren eine separate Linie anzeigen, um einen schnellen Vergleich zu ermöglichen. Im Feld „Gruppieren nach“ wählen Sie den Objekttyp selbst aus, um einzelne Linien, Bereiche, Balken, Spalten usw. (abhängig vom Widget-Typ) für jedes Objekt anzuzeigen.  Sie können die Objekte nach jedem in der Liste verfügbaren Attribut gruppieren.  Wenn Sie beispielsweise Daten für ein internes Volume anzeigen, möchten Sie die Daten möglicherweise nach dem Speichernamen gruppieren.

Um die konsolidierten Daten anzuzeigen, entfernen Sie alle Gruppierungsattribute. Standardmäßig wird dann nach „Alle“ gruppiert.

Sie können diese Daten auf verschiedene Arten aggregieren:
...
Beim Betrachten des Dashboards öffnet die Auswahl der Legende für ein beliebiges Widget, dessen Daten nach „Alle“ gruppiert sind, eine Abfrageseite, die die Ergebnisse der ersten im Widget verwendeten Abfrage anzeigt.
...
=== Identifizierung kundenspezifischer Daten

Wenn Sie ein Widget erstellen oder ändern, das auf Ihren eigenen benutzerdefinierten Daten basiert, werden Ihre Daten im Widget möglicherweise nicht korrekt dargestellt, wenn kein identifizierendes Attribut vorhanden ist.  Falls DII das von Ihnen für Ihr Widget ausgewählte Objekt nicht identifizieren kann, wird Ihnen im Bereich „Gruppieren nach“ ein Link zur _Erweiterten Konfiguration_ angezeigt.  Erweitern Sie diese Option, um das Attribut auszuwählen, anhand dessen Sie Ihre Daten identifizieren möchten.

image:unmodeled_data_identify_by.png["Unbekannte Daten über Dropdown-Menü identifizieren"]
...
Sie können Ihre Diagramme noch besser ausrichten, indem Sie Datenpunkte in Minuten-, Stunden- oder Tagesintervalle aggregieren, bevor diese Daten anschließend nach Attributen zusammengefasst werden (falls ausgewählt). Sie können Datenpunkte nach ihrem _Durchschnitt, Maximum, Minimum, ihrer Summe oder ihrer _Anzahl_ aggregieren.
