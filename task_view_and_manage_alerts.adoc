...
Mit Wartungsfenstern können bestimmte Wartungszeiträume geplant werden, in denen Warnmeldungen für ausgewählte Objekte und Metriken unterdrückt werden. Beispielsweise lassen sich Warnmeldungen unterdrücken, die von bestimmten Speichersystemen ausgelöst werden, wenn sich diese Speichersysteme in einem geplanten Upgrade-Zeitraum befinden.

Es besteht die Möglichkeit, während des Zeitraums sowohl Warnungen als auch Benachrichtigungen zu unterdrücken oder nur Benachrichtigungen zu unterdrücken, während Warnungen weiterhin erkannt werden.

image:Maintenance_Windows_example.png["Beispiel für Wartungsfenster, width=400"]
...
Zum Erstellen eines Wartungsfensters:

. Zu *Observability > Alerts > Maintenance Windows* navigieren und auf *+ Maintenance Window* klicken.
. Geben Sie einen *Namen* für das Wartungsfenster ein. Für optimale Ergebnisse sollte der Name aussagekräftig und präzise sein.
. Das *Speichergerät* oder das *Objektattribut* auswählen.
. Filter für den *Monitor oder die Monitore*. Platzhalter werden akzeptiert.
. Wählen Sie die *Startzeit* und die *Endzeit* für das Wartungsfenster aus.
. Wählen Sie aus, ob *Warnungen und Benachrichtigungen* oder *nur Benachrichtigungen* unterdrückt werden sollen (Warnungen werden während des Fensters weiterhin erkannt).
. Mit *Speichern* wird das Wartungsfenster erstellt.
