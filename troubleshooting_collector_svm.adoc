...
'''
*Problem:* Nach Aktivierung der Workload-Sicherheit wird die ONTAP Performance beeinträchtigt: Die Latenz steigt sporadisch stark an, die IOPS sinken sporadisch stark ab.  *Probieren Sie Folgendes aus:* Bei der Verwendung von ONTAP mit Workload Security können gelegentlich Latenzprobleme in ONTAP auftreten.  Dafür gibt es eine Reihe möglicher Gründe, wie im Folgenden erläutert wird:link:https://mysupport.netapp.com/site/bugs-online/product/ONTAP/BURT/1372994["1372994"] , https://mysupport.netapp.com/site/bugs-online/product/ONTAP/BURT/1415152["1415152"] , https://mysupport.netapp.com/site/bugs-online/product/ONTAP/BURT/1438207["1438207"] , https://mysupport.netapp.com/site/bugs-online/product/ONTAP/BURT/1479704["1479704"] , https://mysupport.netapp.com/site/bugs-online/product/ONTAP/BURT/1354659["1354659"] Die Alle diese Probleme wurden in ONTAP 9.13.1 und höher behoben. Es wird dringend empfohlen, eine dieser neueren Versionen zu verwenden.
...
'''
*Problem:* Nach dem Hinzufügen oder Aktualisieren eines ONTAP SVM Data Collectors (Workload Security) wird beim Mandanten eine Warnung wegen Überprovisionierung oder Abonnementverletzung angezeigt. Die Rohspeicherkapazität hat sich erhöht, obwohl keine neue Hardware hinzugefügt wurde.

*Probieren Sie Folgendes:* Derselbe Speicher kann zweimal gezählt werden, einmal von einem Observability ONTAP Cluster Collector und ein weiteres Mal von einem Workload Security SVM Collector auf demselben Cluster.

Vergleichen Sie die von den Observability ONTAP Collectors und Workload Security SVM Collectors gemeldete Rohspeicherkapazität. Für jeden SVM Collector prüfen, ob sein übergeordneter Cluster bereits von einem Observability ONTAP Collector überwacht wird. Ist dies der Fall, wird nur die Clusterkapazität für die Abonnementnutzung berücksichtigt.

Es sollte geprüft werden, ob kürzlich eine SVM-Migration zwischen Clustern stattgefunden hat. Nach der Migration einer SVM können die Kapazitätsmetriken weiterhin dem vorherigen Cluster zugeordnet bleiben, bis Workload Security aktualisierte Informationen vom Collector erhält.

Sichergestellt ist, dass der ONTAP SVM Data Collector für die migrierte SVM den Status „RUNNING“ aufweist. Falls erforderlich, wird der Collector neu gestartet, damit Workload Security nach der Migration die korrekten Cluster- und Kapazitätsmetriken erfassen kann.

Wenn sowohl die Cluster- als auch die SVM-Kapazität in der Gesamtsumme erscheinen, wird die Kapazität wahrscheinlich doppelt gezählt. Dies tritt häufig kurz nach dem Hinzufügen, Aktualisieren, Neustarten eines SVM-Collectors oder der Migration einer SVM zwischen Clustern auf.

Sollte die Nutzung weiterhin Ihr Abonnement überschreiten, öffnen Sie einen NetApp Supportfall mit Angaben zur Abonnementnutzung vor und nach der Warnung sowie einer Liste der Observability- und SVM-Collector mit deren gemeldeter Rohspeicherkapazität.

{leer}

'''
*Problem*: Bei einem oder mehreren Datensammlern wird „Beeinträchtigt“ anstelle von „Wird ausgeführt“ angezeigt, obwohl der Sammlerdienst aktiv ist.

*Probieren Sie Folgendes aus*: SWS meldet den Status "Degraded", wenn eine oder mehrere FPolicy-Verbindungen auf einem Knoten getrennt sind. Dies ist kein Ausfall, der Collector läuft weiterhin, aber Audit-Ereignisse von diesen Knoten können verloren gehen, bis die Verbindung wiederhergestellt ist.

ONTAP stellt von jedem Knoten, der über eine betriebsbereite Daten-LIF für die SVM verfügt, eine FPolicy Steuerkanalverbindung her. Ein Knoten ohne lokale Daten-LIF kann keine Verbindung herstellen. Siehe:

* link:https://docs.netapp.com/us-en/ontap/nas-audit/node-fpolicy-server-communication-process-concept.html["Kommunikationsprozess zwischen Knoten und externem ONTAP FPolicy Server"^]
* link:https://docs.netapp.com/us-en/ontap/nas-audit/fpolicy-external-fpolicy-servers-concept.html["Wie ONTAP FPolicy mit externen FPolicy Servern funktioniert"^]
* link:https://docs.netapp.com/us-en/ontap/nas-audit/requirements-best-practices-fpolicy-concept.html["Anforderungen, Überlegungen und bewährte Verfahren für die Konfiguration von ONTAP FPolicy"^]


In der Benutzeroberfläche ist zu beachten, welche Knoten nicht verbunden sind, anschließend ist zu prüfen, ob diese Knoten über eine Daten-LIF für die SVM verfügen.

* Keine Daten-LIF auf den aufgeführten Knoten: Der Status „Beeinträchtigt“ kann ignoriert werden. ONTAP kann FPolicy als getrennt anzeigen mit der Begründung „Keine lokale LIF vorhanden, um eine Verbindung zum FPolicy Server herzustellen“, wenn keine lokale Daten-LIF vorhanden ist; dies ist erwartungsgemäß. Siehe link:https://kb.netapp.com/on-prem/ontap/da/NAS/NAS-KBs/Fpolicy_server_not_connected_due_to_no_local_lif_present_on_one_or_more_nodes["KB: Fpolicy Server nicht verbunden, da „kein lokales lif vorhanden“"^]. In einer zukünftigen SWS-Version wird der Collector in diesem Fall nicht mehr als „Beeinträchtigt“ markiert.
* Data LIF auf den aufgeführten Knoten vorhanden: Untersuchen, warum FPolicy getrennt ist (LIF ausgefallen / fehlender data-fpolicy-client, Netzwerk-/Firewallproblem oder temporärer Neustart/Failover).
