...
=== KI-Assistent Visualisierungen

Komplexe Daten werden im Handumdrehen in grafische Erkenntnisse umgewandelt. Der KI-Assistent kann gebeten werden, ein Dashboard zu erstellen, woraufhin die Antwort mit interaktiven Grafiken und Tabellen visualisiert wird. So lassen sich Trends erkennen und Probleme schneller lösen, ohne dass eine manuelle Diagrammerstellung erforderlich ist.

image:ai_assistant-column_graph.png["KI Assistant Visualisierung"]



[[enhanced-vm-analyzer-filtering]]
=== Erweiterte VM Analyzer Filterung

Eine neue Option in der VM Analyzer-Ansicht ermöglicht es, eingebundene Volumes ohne Datenspeicher auszublenden und bietet so eine übersichtlichere Darstellung, die die Fehlersuche bei VMs vereinfacht.

image:vm_analyzer_datastore_only_view.png["Schieberegler für die reine Datastore-Ansicht"]



[[ui-update-user-data-edit-mode]]
=== UI Update: Benutzerdaten Editiermodus

Die Genauigkeit der Anlageninformationen bleibt erhalten, während das Risiko unbeabsichtigter Änderungen verringert wird. Anmerkungen und Attribute (d. h. „Benutzerdaten“) erscheinen standardmäßig im Nur-Lese-Modus, sodass zunächst die Details geprüft werden können und anschließend entschieden werden kann, wann eine Bearbeitung erfolgen soll.

Nur bei Bedarf bearbeiten. In einem ausklappbaren Bedienfeld lassen sich die Asset-Details erweitern und das Stiftsymbol auswählen. Die Asset-Startseite öffnet sich im Bearbeitungsmodus, in dem die Informationen aktualisiert werden können und Speichern ausgewählt werden kann, sobald Sie fertig sind.

Sie können auch direkt die Landingpage des Assets öffnen und oben rechts „Bearbeiten“ auswählen.

image:UserDataEdit-Slideout.png["Benutzerdaten im Slideout Panel mit Stiftsymbol bearbeiten, Breite=400"]

image:UserDataEdit-LP.png["Landing Page Bearbeitungsmodus, Breite=200"]
...
=== Neue KI-Assistent Startseite

Der DII KI-Assistent verfügt nun über eine eigene Startseite, die die intelligente Infrastrukturanalyse in den Mittelpunkt stellt. Neue Konversationen können sofort begonnen, die letzte Sitzung mit einem Klick fortgesetzt oder ein schneller Einstieg mit vorgeschlagenen Eingabeaufforderungen genutzt werden.

image:AI_dashboard_page_default.png["KI Dashboard Standard, Breite=800"]

Auf die Startseite kann über den *AI Assistant* im Navigationsmenü zugegriffen werden oder der AI Assistant kann von überall aus über das „Sterne“-Symbol oben links geöffnet werden.

image:AI_dashboard_page_menu.png["KI Assistant Navigationsmenü, Breite=600"] image:AI_assistant_access_button.png["KI-Assistenten-Zugriff von überall, Breite=300"]
...
=== Wichtige Aktualisierungen mit Anmeldenachrichten teilen

Das Team bleibt mit anpassbaren Anmeldenachrichten informiert und abgestimmt. Wichtige Ankündigungen, Wartungspläne oder Richtlinienaktualisierungen werden direkt bei der Anmeldung an die Benutzer kommuniziert. Mit Markdown-Unterstützung lassen sich Nachrichten übersichtlich formatieren, sodass kritische Informationen jeden DII-Benutzer erreichen. Konten mit Administratorrechten finden die Konfiguration im Admin-Menü unter Benachrichtigungen.



[[more-workspace-less-clutter]]
=== Mehr Arbeitsfläche, weniger Unordnung

Das DII-Navigationsmenü präsentiert sich in einem neuen Look, der farblich und funktional an die Console angepasst ist und so für ein einheitlicheres Benutzererlebnis sorgt. Das bisher statische Hauptmenü ist standardmäßig ausgeblendet, um einen übersichtlicheren Arbeitsbereich und eine effizientere Nutzung des Bildschirmplatzes zu ermöglichen. Klicken Sie einfach auf das Hamburger-Symbol (☰) oben links, um es aufzurufen.



[[expand-device-visibility-with-intelligent-ai-assistance]]
=== Erweitern Sie die Gerätetransparenz mit intelligenter KI-Unterstützung

Beschleunigen Sie die Integration neuer Geräte mit KI-gestützter Geräteidentifizierung. Data Infrastructure Insights analysiert Ihre Umgebung und schlägt Regeln zur automatischen Geräteerkennung vor, damit Sie Geräte schnell identifizieren und verwalten können. Prüfen und genehmigen Sie einfach die KI-generierten Regeln und DII kümmert sich um den Rest, wodurch die Transparenz und Verwaltung Ihrer Geräte in Ihrer gesamten Infrastruktur verbessert wird.



[[a-smarter-ai-assistant-for-global-teams]]
=== Ein intelligenterer KI-Assistent für globale Teams

Der KI-Assistent unterstützt jetzt über ein Dutzend Sprachen und ermöglicht Ihrem Team so die natürliche Kommunikation in der jeweiligen Muttersprache. Die vollständige Liste finden Sie hier link:https://platform.claude.com/docs/en/build-with-claude/multilingual-support["hier,"^].



[[new-monitors-for-infrastructure-attribute-changes]]
=== Neue Überwachungsfunktionen für Änderungen der Infrastrukturattribute

Attributmonitore sind jetzt verfügbar, um Ihr Team automatisch zu benachrichtigen, wenn Assets zwischen Rechenzentren verschoben werden, ein Speicherpool nicht verschlüsselt ist oder Attributänderungen vorgenommen werden. Dies trägt zur Einhaltung der Konfigurationsbaselines bei und hilft sicherzustellen, dass bei neuen Bereitstellungen oder Änderungen nichts übersehen wird.

image:attribute_monitor_in_list.png["Attributmonitore werden zu Monitortypen hinzugefügt"]



[[build-smarter-alerts-and-webhooks-with-dynamic-context-rich-messages]]
=== Erstellen Sie intelligentere Benachrichtigungen und Webhooks mit dynamischen, kontextreichen Nachrichten

Beim Erstellen eines Monitors oder Webhooks geben Sie einfach „%“ in der Alarmbeschreibung, der Webhook-URL, im Header oder im Body ein, um aus einer dynamisch gefüllten Liste verfügbarer Objektattribute und Annotationen auszuwählen. Geben Sie Ihren Alarmen den richtigen Kontext, um die Verständlichkeit zu verbessern, die Webhook-Integration zu verbessern und die Reaktionszeiten zu verkürzen.

image:attribute_list_example.png["Attributliste in einer Monitorbeschreibung, Breite=400"]
...
=== DII kennt Ihre Umgebung besser als je zuvor

Wir haben Verbesserungen daran vorgenommen, wie die KI in Data Infrastructure Insights Ihre Einrichtung versteht. Sie greift nun auf umfassendere Kontextinformationen zu Ihrer aktuellen Nutzung des Dienstes, der Konfiguration und den Leistungsmustern zurück, bevor sie antwortet. Das Ergebnis ist eine erste Anleitung, die Ihre Umgebung widerspiegelt, anstatt nur einen generischen Ausgangspunkt zu bieten, sodass weniger Nachfragen nötig sind, um zu einer hilfreichen Antwort zu gelangen.

Navigieren Sie beispielsweise zu einem Speichermedium, und der KI-Assistent liefert mit einem Klick Antworten auf Fragen wie:

* Wie entwickelt sich die Kapazitätsauslastung dieses Speichers, und welche StoragePools belegen den meisten Speicherplatz?
* Können Sie mir den Kapazitätsauslastungstrend für dieses Gerät in den letzten 24 Stunden zeigen?
* Welche Volumes belegen den meisten Speicherplatz auf diesem Storage?
...
=== AI Assistant ist jetzt allgemein verfügbar

Der Data Infrastructure Insights AI Assistant ist jetzt allgemein für alle Kunden für DII-Sites, die auf AWS gehostet werden, verfügbar.

Bitte beachten Sie, dass sich der KI-Assistent auf Azure sites noch in der Vorschauphase befindet. Beobachten Sie diesen Bereich für die Ankündigung der allgemeinen Verfügbarkeit.



[[infrastructure-insights-features-removed]]
=== Infrastructure Insights-Funktionen entfernt

Die „Infrastructure Insights“-Funktionen von DII _Shared Resources Under Stress_, _Kubernetes Namespaces Running out of Space_ und _Reclaiming ONTAP Cold Storage_ sind in DII nicht mehr verfügbar.
...
=== Sofort intelligente Antworten zu Ihrer Infrastruktur

Lernen Sie den neuen intelligenten Partner Ihrer Infrastruktur kennen: Die link:ai_for_dii_overview.html["*Data Infrastructure Insights KI-Assistent*"] transformiert die Interaktion mit Ihren Daten und macht aus komplexen Abfragen einfache Gespräche. Entdecken Sie sofort verborgene Muster, beschleunigen Sie die Fehlersuche und treffen Sie datenbasierte Entscheidungen mit Vertrauen. Erhalten Sie sofort Antworten auf Fragen wie:
...
Die DII AI Assistant Preview ist ab sofort für die meisten Kunden mit in AWS-Regionen gehosteten Diensten verfügbar; die Azure-Region folgt in Kürze. link:concept_preview_features.html["Mehr erfahren"] über die Data Infrastructure Insights Preview-Funktionen.

image:ai_assistant_icon.png["KI-Assistent-Symbol"]
...
==== Systemmonitore schneller anpassen, ohne zu duplizieren

Passen Sie die Standard-Systemmonitore direkt an Ihre Bedürfnisse an – ändern Sie Namen, Filter und Gruppierungen an einem Ort. Sie müssen keine doppelten Monitore für einfache Anpassungen erstellen. Ein hilfreiches Banner erinnert Sie daran, welche Monitore angepasst wurden, sodass Sie stets den vollen Überblick über Ihre Überwachungskonfiguration behalten.



==== Monitorverwaltung mit Massenaktionen optimieren

Verwalten Sie mehrere Monitore gleichzeitig mit Massenaktionen. Passen Sie die Einstellungen für die Health Monitoring für Gruppen von Monitoren schnell an, sparen Sie Zeit und reduzieren Sie die Komplexität. Aktualisieren Sie Ihre Überwachungsstrategie flächendeckend in Sekunden, nicht in Minuten.



==== Konzentrieren Sie Ihre Infrastruktur-Gesundheitsbewertungen auf das, was am wichtigsten ist

Schließen Sie bestimmte Objekte von Ihren Infrastructure Health-Bewertungen aus, um Genauigkeit und Relevanz zu gewährleisten. Wenn Sie Ihr Infrastructure Health-Dashboard anzeigen, können Sie Objekte, deren Priorität Sie herabstufen möchten, sofort entfernen, ohne die Monitor-Konfiguration zu ändern. So konzentrieren Sie Ihre Warnmeldungen auf die Infrastrukturelemente, die Ihre Geschäftsprioritäten bestimmen.

image:health_monitoring_exclude_object.png["Objekt von der Gesundheitsüberwachung ausschließen"]
...
Die folgenden Linux-Betriebssysteme werden derzeit von Data Infrastructure Insights link:concept_acquisition_unit_requirements.html["Akquisitionseinheiten"] unterstützt:

* AlmaLinux 9.5 und 10
* Centos (64-Bit): Stream 9
* Debian (64-bit): 11 bis 13
* OpenSUSE Leap 15.2 bis 15.6, 16
* Oracle Enterprise Linux (64-bit): 8.10, 9.2, 9.4, 9.6, 10
* Red Hat (64-bit): 8.10, 9.2, 9.4, 9.6, 10
* Rocky 9.2 bis 9.6
* SUSE Linux Enterprise Server 15 SP4 bis 15 SP7, 16
...
=== Zugriff auf Abfragen mit freigegebenen Abfragen steuern

Teilen Sie Ihre erstellten Abfragen mit fein abgestuften Berechtigungssteuerungen. Wählen Sie den schreibgeschützten Anzeigemodus, um Änderungen zu verhindern, oder den Bearbeitungsmodus, um Änderungen zu ermöglichen. Private Abfragen sind nur für die Ersteller zugänglich, während gemeinsam genutzte Abfragen in Annotationsregeln verwendet oder für spezielle Anwendungsfälle dupliziert werden können. Dadurch haben Sie die volle Kontrolle darüber, wie Abfragen in Ihrer Organisation verwendet und geändert werden.

image:query_sharing_dialog.png["Berechtigungen für eine Abfrage festlegen, Breite=200"]

Darüber hinaus können Benutzer mit der Rolle „Gast“ nun ihre eigenen privaten Abfragen erstellen und verwalten.



[[better-filtering-when-managing-monitors]]
=== Verbesserte Filterung bei der Monitorverwaltung

Die Filterung von Monitorlisten und -gruppen wurde verbessert und bietet nun Unterstützung für Platzhalter, mehrere Filter, Ein-/Ausschlussfunktionen und mehr.

image:monitor_list_filters.png["Neue Monitorfilterung, Breite=500"]



[[cleaner-dashboard-visualizations]]
=== Übersichtlichere Dashboard-Visualisierungen

Das verbesserte Dashboard-Design hilft Ihnen, das Wichtigste im Blick zu behalten. Verbesserte visuelle Klarheit durch Schlagschattenränder, die Widgets voneinander trennen, und entfernte Trennlinien, um Unordnung zu vermeiden, damit Sie sich auf Ihre Dateneinblicke konzentrieren können. Diese Verbesserungen schaffen ein übersichtlicheres und intuitiveres Dashboard-Erlebnis, das das Erkennen von Trends und Anomalien schneller und einfacher macht.
...
Reclaim ONTAP Cold-Storage Insight unterstützt jetzt FlexGroups und ist ab sofort für alle Kunden verfügbar.
...
| Kubernetes-Namespaces haben nicht mehr genügend Speicherplatz | Der Insight „Kubernetes Namespaces Running Out of Space“ bietet eine Ansicht der Workloads in Ihren Kubernetes Namespaces, bei denen das Risiko besteht, dass der Speicherplatz knapp wird, einschließlich einer Schätzung der verbleibenden Tage, bevor jeder Bereich voll ist. 


| Gemeinsam genutzte Ressource unter Stress | Die Erkenntnis „_Shared Resource Under Stress_“ nutzt KI/ML, um automatisch zu erkennen, wo Ressourcenkonflikte zu Leistungseinbußen in Ihrer Umgebung führen, hebt alle davon betroffenen Workloads hervor und bietet empfohlene Maßnahmen zur Behebung, sodass Leistungsprobleme schneller gelöst werden können. 
...
Cloud Insights stellt regelmäßig eine Reihe spannender neuer Funktionen in der Vorschau vor. Bei Interesse an einer oder mehreren dieser Funktionen kann das NetApp Sales Team für weitere Informationen kontaktiert werden.
...
| Gemeinsam genutzte Ressource unter Stress | Die Funktion „Shared Resource Under Stress“ nutzt KI/ML, um automatisch zu erkennen, wo Ressourcenkonflikte die Performance in Ihrer Umgebung beeinträchtigen, hebt alle davon betroffenen Workloads hervor und bietet empfohlene Maßnahmen zur Behebung, sodass Performanceprobleme schneller gelöst werden können. 
|===
...
'''nt1.png[Verwaltung von Tabellen-Widget-Einheiten]
