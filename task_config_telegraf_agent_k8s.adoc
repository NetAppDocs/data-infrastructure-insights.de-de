...
NOTE: Für Bedienerversionen vor 1.2057.0 sind keine Aktualisierungen per Knopfdruck möglich. Bitte befolgen Sie die unten stehenden Anweisungen für die manuelle Aktualisierung, um auf die neueste Version zu aktualisieren. Anschließend können zukünftige Aktualisierungen per Knopfdruck durchgeführt werden.
...
| „ImagePullBackoff“-Fehler aufgetreten | Diese Fehler können auftreten, wenn Sie ein benutzerdefiniertes oder privates Docker-Repository verwenden und den Kubernetes Monitoring Operator noch nicht so konfiguriert haben, dass er dieses korrekt erkennt. <<Verwenden eines benutzerdefinierten oder privaten Docker-Repositorys,Mehr lesen>> Informationen zur Konfiguration für benutzerdefinierte/private Repositories. 
...
| Ich erhalte Fehlermeldungen von Telegraf, die der folgenden ähneln, aber Telegraf startet und läuft trotzdem: E! [inputs.kubernetes] Fehler im Plugin: https://<IP>>:<port>>/pods[] returned HTTP status 403 Forbidden | Diese Meldungen können auftreten, wenn Sie Kubernetes-Versionen unter 1.33 verwenden. Die von telegraf verwendete RBAC-Ressource „nodes/pods“ ist in diesen Versionen nicht vorhanden. Im Abschnitt „For Kubernetes versions < 1.33“ der Datei operator-additional-permissions.yaml sind die für ältere Versionen erforderlichen RBAC-Ressourcen aufgeführt. 
|===
