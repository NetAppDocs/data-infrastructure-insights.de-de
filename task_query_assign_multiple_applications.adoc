...
summary: Anwendungen und Annotationen können effizient über mehrere Assets hinweg verwaltet werden, indem abfragebasierte Massenoperationen zum Zuweisen oder Entfernen genutzt werden, ohne dass eine manuelle, individuelle Asset-Konfiguration erforderlich ist. 
...
[role="lead"]
Die Asset-Verwaltung lässt sich optimieren, indem Abfragen verwendet werden, um Massenoperationen an Anwendungen und Annotationen durchzuführen. Anstatt jedes Asset einzeln manuell zu konfigurieren, besteht die Möglichkeit, mehreren Anwendungen und Annotationen gruppenweise in einem einzigen Vorgang effizient zuzuweisen oder zu entfernen.
