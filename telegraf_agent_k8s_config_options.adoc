...
[role="lead"]
Der link:task_config_telegraf_agent_k8s.html#configuringcustomizing-the-operator["Kubernetes Monitoring Operator"] Bietet umfangreiche Anpassungsmöglichkeiten über die AgentConfiguration-Datei. Konfigurieren Sie Ressourcenlimits, Erfassungsintervalle, Proxy-Einstellungen, Toleranzen und komponentenspezifische Einstellungen für Telegraf, Kube-State-Metrics, Protokollerfassung, Workload-Mapping, Änderungsmanagement und mehr, um die Überwachung Ihrer Kubernetes-Umgebung zu optimieren.
...
|  | Ungeschützt | ee dieses link:https://community.influxdata.com/t/updating-telegraf-to-version-1-29-5-crashes-kubernetes-pod/33376["Bekanntes Problem mit Telegraf"]. Durch die Einstellung _Unprotected_ wird der Kubernetes Monitoring Operator angewiesen, Telegraf mit dem Flag auszuführen `--unprotected`. 
