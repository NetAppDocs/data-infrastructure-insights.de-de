...
[role="lead"]
Workload Security (ehemals Cloud Secure) erfasst Daten zu Benutzeraktivitäten mithilfe eines oder mehrerer Agenten. Mitarbeiter stellen Verbindungen zu Geräten in Ihrer Umgebung her und erfassen Daten, die zur Analyse an die SaaS-Ebene für die Workload-Sicherheit gesendet werden. Informationen zum Konfigurieren einer Agent-VM finden Sie unterlink:concept_cs_agent_requirements.html["Anforderungen An Den Agenten"].
