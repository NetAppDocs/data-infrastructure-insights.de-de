...
summary: Systemdefinierte Monitore für Metrik-, Protokoll-, Attribut- und Datenerfassungsbedingungen werden angezeigt. 
---
= Systemmonitore
:hardbreaks:
:allow-uri-read: 
...
[role="lead"]
Data Infrastructure Insights umfasst systemdefinierte Monitore für Metriken, Protokolle, Attribute und Datenerfassungsbedingungen. Die verfügbaren Systemmonitore hängen von den in Ihrem Mandanten vorhandenen Datensammlern ab. Aus diesem Grund können sich die in Data Infrastructure Insights verfügbaren Monitore ändern, wenn Datensammler hinzugefügt oder deren Konfigurationen geändert werden.
...
Systemdefinierte Monitore bestehen aus vordefinierten Metriken oder Attributen und Bedingungen sowie Standardbeschreibungen und Korrekturmaßnahmen, die nicht geändert werden können. Die Empfängerliste für Benachrichtigungen systemdefinierter Monitore kann jedoch bearbeitet werden. Die Metriken oder Attribute, Bedingungen, Beschreibungen und Korrekturmaßnahmen sowie die Möglichkeit, die Empfängerliste zu ändern, stehen zur Verfügung, wenn eine Gruppe systemdefinierter Monitore geöffnet und in der Liste auf den Monitornamen geklickt wird.
...
* *CVO Infrastructure* und *CVO Workload Examples* beinhalten entsprechende Monitore für Cloud Volumes ONTAP.
* Der Standardzustand variiert je nach Monitor. In den folgenden Tabellen kann der jeweilige Monitor vor der Aktivierung von Benachrichtigungen überprüft werden.
...
| Kubernetes Persistent Volume Reclamation fehlgeschlagen | Die automatische Wiederherstellung des Datenträgers ist fehlgeschlagen. |  | Warnung @ > 0 B 
...
| Kubernetes Pod-Crash-Loop-Backoff | Pod ist abgestürzt und hat mehrere Neustartversuche unternommen. |  | Kritisch @ > 3 
...
| Kubernetes Job fehlgeschlagen | Der Auftrag wurde aufgrund eines Knotenabsturzes oder Neustarts, einer Ressourcenerschöpfung, einer Auftragszeitüberschreitung oder eines Pod-Planungsfehlers nicht erfolgreich abgeschlossen. | Überprüfen Sie die Kubernetes-Ereignisprotokolle auf Fehlerursachen. | Warnung @ > 1 
...
| Entdecktes Volume | Informativ | Diese Meldung wird angezeigt, wenn ein internes Volume erkannt wird. 


| Volumen geändert | Informativ | Diese Meldung wird angezeigt, wenn ein internes Volume geändert wird. 


| Knoten entdeckt | Informativ | Diese Meldung wird angezeigt, wenn ein Speicherknoten erkannt wird. 


| Knoten entfernt | Informativ | Diese Meldung wird angezeigt, wenn ein Speicherknoten entfernt wird. 
...
Attributmonitore werten die für ein Objekt gemeldeten Konfigurationsattribute aus. Im Gegensatz zu Metrikmonitoren vergleichen sie keine Zeitreihenmetrik mit einem numerischen Schwellenwert. Jede der folgenden Bedingungen generiert eine _Warning_-Warnmeldung, wenn das gemeldete Attribut einem nicht konformen oder nicht verfügbaren Zustand entspricht.

Die bisherigen eigenständigen ONTAP Essentials Sicherheitsmetrikmonitore wurden durch die konsolidierten Attributmonitore *ONTAP Cluster Misconfigured* und *ONTAP Storage Misconfigured* ersetzt. Die entsprechenden CVO-Monitore wenden dieselben Prüfungen auf Cloud Volumes ONTAP an.



==== ONTAP Attributmonitore

|===
| Monitorname | Gruppe | Standardzustand | Bedingungen | Korrekturmaßnahme 


| ONTAP Cluster falsch konfiguriert | ONTAP Infrastruktur | Aktiv | Warnungen, wenn eine oder mehrere der folgenden 12 Sicherheitsbedingungen erkannt werden: * AutoSupport HTTPS ist nicht konfiguriert * Web HTTP ist aktiviert * SSH ist nicht gesichert * Das Anmeldebanner ist deaktiviert * Die Cluster-Peer-Kommunikation ist nicht verschlüsselt * Der Zertifikatsaussteller ist selbstsigniert * Der Standard-Lokaladministrator ist aktiviert * FIPS ist deaktiviert * Die Protokollweiterleitung ist nicht verschlüsselt * MD5-Passwort-Hashing ist aktiviert * Remote Shell (RSH) ist aktiviert * Telnet ist aktiviert | Die in der Warnmeldung aufgeführten betroffenen Attribute werden angezeigt. Unsichere Protokolle und Passwort-Hashing können deaktiviert, SSH und die Peer-Kommunikation gesichert, die erforderlichen Sicherheitskontrollen aktiviert und selbstsignierte Zertifikate gemäß den Sicherheitsrichtlinien Ihrer Organisation ersetzt werden. 


| ONTAP Storage falsch konfiguriert | ONTAP Infrastruktur | Aktiv | Warnmeldungen, wenn bei einer ONTAP Storage VM die Protokollierung deaktiviert ist, SSH nicht gesichert ist oder das Anmeldebanner deaktiviert ist. | Die Überwachungsprotokollierung der Storage-VM, die Absicherung von SSH sowie die Konfiguration eines geeigneten Anmeldebanners sind zu berücksichtigen. 


| ONTAP Cluster fehlen Ersatzfestplatten | ONTAP Infrastruktur | Aktiv | Benachrichtigungen, wenn der Cluster meldet, dass nur noch wenige Ersatzfestplatten vorhanden sind. | Ersatzfestplatten werden pro Knoten überprüft und kompatible Ersatzkapazität wird hinzugefügt, um den Datenschutz aufrechtzuerhalten. 


| ONTAP Storage Service nicht verfügbar | ONTAP Workload Beispiele | Aktiv | Warnmeldungen, wenn ein aktivierter FCP- oder iSCSI-Service auf einer ONTAP Storage VM nicht verfügbar ist. | Status der Storage VM, des Protokolls und der Daten-LIF überprüfen. Die letzten ONTAP Health- und Ereignismeldungen prüfen und den betroffenen SAN Service wiederherstellen. 


| ONTAP Node falsch konfiguriert | ONTAP Workload Beispiele | Angehalten | Warnmeldungen, wenn Cisco Discovery Protocol (CDP) auf einem ONTAP Node deaktiviert ist. | Die CDP-Einstellung des Knotens und der Status des Netzwerkports sollten überprüft werden, anschließend ist CDP dort zu aktivieren, wo es für die Netzwerkerkennung und die Topologieüberwachung erforderlich ist. 
|===


==== Cloud Volumes ONTAP Attributmonitore

|===
| Monitorname | Gruppe | Standardzustand | Bedingungen | Korrekturmaßnahme 


| CVO Cluster falsch konfiguriert | CVO Infrastruktur | Aktiv | Wendet die für *ONTAP Cluster Misconfigured* beschriebenen Cluster-Sicherheitskonfigurationsprüfungen auf Cloud Volumes ONTAP an. | Die betroffenen Attribute und die gemeldeten unsicheren oder nicht konformen Cluster-Einstellungen werden überprüft und gegebenenfalls korrigiert. 


| CVO Storage falsch konfiguriert | CVO Infrastruktur | Aktiv | Warnmeldungen, wenn bei einer Cloud Volumes ONTAP Storage VM die Protokollierung deaktiviert ist, SSH nicht gesichert ist oder das Anmeldebanner deaktiviert ist. | Die Überwachungsprotokollierung der Storage-VM, die Absicherung von SSH sowie die Konfiguration eines geeigneten Anmeldebanners sind zu berücksichtigen. 


| CVO Node falsch konfiguriert | CVO Workload-Beispiele | Angehalten | Warnmeldungen, wenn CDP auf einem Cloud Volumes ONTAP Knoten deaktiviert ist. | Die CDP- und Netzwerkeinstellungen des Knotens sollten überprüft und CDP bei Bedarf aktiviert werden. 
|===


==== NTP-Konfigurationsmonitore

Die folgenden NTP-Monitore stehen weiterhin als eigenständige, systemdefinierte Monitore für ONTAP und Cloud Volumes ONTAP zur Verfügung.

|===
| Monitorname | Plattform | Schwelle | Monitorbeschreibung | Korrekturmaßnahme 


| Es sind keine NTP-Server konfiguriert | ONTAP | Warnung @ < 1 | Der Cluster verfügt über keine konfigurierten NTP-Server. Für Redundanz und optimalen Service empfiehlt NetApp, dem Cluster mindestens drei NTP-Server zuzuordnen. | Einem Cluster wird ein NTP-Server mithilfe von  `cluster time-service ntp server create -server <ntp_server>` zugeordnet. 


| Die Anzahl der NTP-Server ist niedrig | ONTAP | Warnung @ < 3 | Der Cluster verfügt über weniger als drei konfigurierte NTP-Server. | Zusätzliche NTP-Server werden mit dem Cluster assoziiert, indem  `cluster time-service ntp server create -server <ntp_server>` verwendet wird. 


| CVO Keine NTP-Server sind konfiguriert | Cloud Volumes ONTAP | Warnung @ < 1 | Der Cluster verfügt über keine konfigurierten NTP-Server. | Einem Cluster wird ein NTP-Server mithilfe von  `cluster time-service ntp server create -server <ntp_server>` zugeordnet. 


| Die Anzahl der CVO NTP Server ist gering | Cloud Volumes ONTAP | Warnung @ < 3 | Der Cluster verfügt über weniger als drei konfigurierte NTP-Server. | Zusätzliche NTP-Server werden mit dem Cluster assoziiert, indem  `cluster time-service ntp server create -server <ntp_server>` verwendet wird. 
|===
...
| MetroCluster FC-Initiatorverbindung unterbrochen | KRITISCH | Der FC-Initiatoradapter ist fehlerhaft. | 1. Stellen Sie sicher, dass die FC-Initiatorverbindung nicht manipuliert wurde. 2.  Überprüfen Sie den Betriebsstatus des FC-Initiatoradapters mit dem Befehl „system node run -node local -command storage show adapter“. 


| MetroCluster FC-VI Interconnect-Link ausgefallen | KRITISCH | Die physische Verbindung am FC-VI-Port ist offline. | 1. Stellen Sie sicher, dass die FC-VI-Verbindung nicht manipuliert wurde. 2.  Überprüfen Sie mit dem Befehl „metrocluster interconnect adapter show“, ob der physische Status des FC-VI-Adapters „Aktiv“ lautet. 3.  Wenn die Konfiguration Fabric-Switches umfasst, stellen Sie sicher, dass diese ordnungsgemäß verkabelt und konfiguriert sind. 
